...
def find_documents_fields(database_name, collection_name, query=None, fields=None, sort=None):
    if query is None:
        query = {}
    db = get_db(database_name)
    collection = db[collection_name]
...
def find_documents_all(database_name, collection_name, query=None):
    if query is None:
        query = {}
    db = get_db(database_name)
    collection = db[collection_name]
